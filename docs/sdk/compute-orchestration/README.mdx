...
Also, set up a sub-folder named `configs` to store the YAML configuration files needed for Compute Orchestration.
...
Then, to create a new deployment, provide the `deployment_id` and `config_filepath` parameters to the `create_deployment` method of the `Nodepool` class.
...
To delete compute clusters, supply a list of compute cluster IDs to the `delete_compute_clusters` method of the `User` class.
