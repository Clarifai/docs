...
Below is an example of performing patch operations on a workflow, where the YAML configuration is updated, and changes are made to its description, notes, and image URL. Note that the `remove` action is only used to delete the workflow's cover image on the platform UI.
