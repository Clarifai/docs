...
|![improvement](/img/improvement.jpg)| Introduced multi-label text classification with Hugging Face text classification pipeline  | <ul><li> Previously, there was an issue with multi-label text classification using the Hugging Face text classification pipeline. While it worked well for multi-class classification, it did not perform correctly for multi-label classification. We fixed the issue. </li></ul> |
