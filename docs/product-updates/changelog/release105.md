...
|![improvement](/img/improvement.jpg)|Redesigned the user activation form for new Clarifai accounts|After signing up, users receive a verification email. Clicking the link in the email redirects them to the Clarifai platform to complete their profile details.<br/><br/>Key new features of the activation form include: <br/><br/><ul><li>The ability to join an existing organization if the user's email domain matches the organization's domain.</li><li>The option to create a new organization and invite members to join.</li></ul>|
...
|![improvement](/img/improvement.jpg)|Changed the default base workflow to Universal| <ul><li>Previously, when an application was created via the API without specifying a base workflow, the General workflow was used by default. Now, the default has been updated to the Universal workflow, ensuring consistency with applications created via the UI.</li></ul>|
