...
- `--pool_size` is an integer option that controls the number of threads used to process inference requests (default: 32). Increasing the pool size allows more requests to be handled in parallel, which can improve throughput and speed up local testing, at the cost of higher compute usage.

- `--suppress-toolkit-logs` option enables detailed display of logs for troubleshooting issues, especially when running an [Ollama server](https://docs.clarifai.com/compute/local-runners/ollama/#step-4-start-your-local-runner). By default, logs are suppressed.

- `--mode [env|container|none]` specifies how the model is executed locally. Use `env` to run the model inside a temporarily created virtual environment, `container` to run it in a Docker container, or `none` to run the model directly without creating an isolated environment. The default mode is `none`.

- `--keep_image` retains the automatically created Docker image after the local test run completes. This option applies only when using `container` mode and defaults to `False`.
