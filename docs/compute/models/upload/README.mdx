...
`git clone https://github.com/Clarifai/examples.git`. After cloning it, go to the `models/model_upload` directory.
...
You need to include at least one method decorated with `@ModelClass.method` to define the [prediction endpoints](https://docs.clarifai.com/compute/models/model-inference).
...
You can find more details on making predictions [here](https://docs.clarifai.com/compute/models/model-inference). 
...
Once the model is successfully uploaded to Clarifai, you can start making [predictions](https://docs.clarifai.com/compute/models/model-inference) with it. Note that before making a prediction request with our Compute Orchestration capabilities, you need to first deploy it into a cluster and nodepool you've created. 
...
You can refer to [this examples repository](https://github.com/Clarifai/examples/tree/main/models/model_upload/llms) for additional examples of uploading other types of LLMs.
