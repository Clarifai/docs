...
| 21353 | MODEL_NOT_DEPLOYED | Used when model spire deployment is manually taken down or due to inactivity |
 |21354 | MODEL_BUSY_PLEASE_RETRY | The model is currently saturated with active requests. Incoming requests are queued, and if the model does not become available within a certain time, this status is returned. Retry the request after a short delay.|
