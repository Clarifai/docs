...
If you check the [model's versions table](https://docs.clarifai.com/portal-guide/model-versions/#model-versions-table), you’ll notice that the model is still being trained. Many hours may be required to deep train models with large numbers of inputs and complex taxonomies. 

You can check the training progress by clicking the **View Training Log** button. You can also cancel a deep fine-tuning job at any time by clicking the **Cancel training** button. 
