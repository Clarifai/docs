...
- **Max aspect_ratio**—When "keep_aspect_ratio" is set to True, it is used to control the maximum length of the longer side of an image relative to the shorter side during image resizing. You can set it by: `{"min": 1.0, "max": 5.0}`.
...
- **Pretrained weights**—This specifies whether to init the model with pre-trained weights. You can set it by: `[{"id":"None"},{"id":"coco"}]`.
- **Frozen stages**—This specifies the backbone network stages to keep frozen during training. You can set it by: `{"min": 1, "max": 4, "step": 1}`.
